...
def process_data(query, title, content, max_seq_len, masking_obj=None):
    ...
    #padding_mask = [1] * len(data)
    if len(data) < max_seq_len:
        #padding_mask += [0] * (max_seq_len - len(data))
...
class DataCollator:
    def __call__(self, features) -> Dict[str, Any]:
        ...
        token_ids = np.array([f['src_input'] for f in features],dtype=np.int64)  #paddle.to_tensor(, dtype="int64")
        segment = paddle.to_tensor([f['segment'] for f in features], dtype="int64")

        #For test data
        batch_data = {
            'input_ids':paddle.to_tensor(token_ids, dtype="int64"),
            'attention_mask':paddle.to_tensor((token_ids > 0).astype('float32'), dtype='float32'),
            'token_type_ids':segment
        }
        #For training data, pointwise or groupwise
        if 'masked_lm_labels' in features[0]:
            masked_lm_labels = paddle.to_tensor([f['masked_lm_labels'] for f in features], dtype="int64")
            batch_data.update({'masked_lm_labels': masked_lm_labels})

        #For testing data
        if 'qid' in features[0]:
            qids = paddle.to_tensor([f['qid'] for f in features], dtype="int64")
            batch_data.update({'qids': qids})

        if 'label' in features[0]:
            labels = paddle.to_tensor([f['label'] for f in features], dtype="int64")
            batch_data.update({'labels': labels})

        if 'freq' in features[0]:
            freqs = paddle.to_tensor([f['freq'] for f in features], dtype="int64")
